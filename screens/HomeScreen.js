...
const HomeScreen = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get("http://http://192.168.1.103:8081/get-posts");
  ...
  const handleLike = async (postId) => {
    try {
      const response = await axios.put(
        `http:/http://192.168.1.103:8081/posts/${postId}/${userId}/like`
      );
  ...
  const handleDislike = async (postId) => {
    try {
      const response = await axios.put(
        `http://http://192.168.1.103:8081/posts/${postId}/${userId}/unlike`
      );
      const updatedPost = response.data;
